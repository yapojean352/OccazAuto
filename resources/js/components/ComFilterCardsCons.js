...
/** nous avons donc décidé que ComFilterCardsCons possèdera notre état.
 *  Tout d’abord, ajoutez une propriété d’instance
 *  this.state = {filterText: '', inStockOnly: false} dans le constructor
 *  de ComFilterCardsCons pour refléter l’état initial de votre application.
...
class ComFilterCardsCons extends Component {
  constructor(props) {
    ...
    // axios.get('http://localhost:8000/api/list/all').then(res =>{
  ...
  render() {
    ...
    return (
      ...
      <div className='catalogue-container'>
           ...
           <div className='columns listing-column'>
                 ...
                 {this.state.listFilter.map((valF,i)=>{
                  ...
                  return(<span  className="applied-filter">Delete All {this.state.listFilter.length}
                    <a onClick={(e)=>this.removeKey('all')} className='close'>X</a></span>)
                 }
                }
                 )}
          ...
          <h5> Example display API gfuh </h5>
...
export default ComFilterCardsCons